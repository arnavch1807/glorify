...
import { Track, Album, Artist, TrackLyrics, GenreCollection } from '@chotify/types';
...
interface LocalLibraryState {
  ...
  localAlbums: Album[];
  localArtists: Artist[];
  localGenres: GenreCollection[];
  folders: LocalFolder[];
  scanningState: 'idle' | 'scanning' | 'ready' | 'error';
  scanProgress: { current: number; total: number };
  isPermissionRequired: boolean;

  loadSavedLibrary: () => Promise<void>;
  requestFolderPermissions: () => Promise<boolean>;
  importDirectory: () => Promise<void>;
  importFileList: (files: FileList | File[]) => Promise<void>;
  deleteLibrary: () => Promise<void>;
  resolveAudioFile: (track: Track) => Promise<File>;
  saveTracksMetadata: (
    trackIds: string[],
    fields: {
      title?: string;
    ...
    newArtworkBlob?: Blob | null,
    removeArtwork?: boolean
  ) => Promise<void>;
  performUndoMetadata: () => Promise<void>;
  saveTrackLyrics: (trackId: string, lyrics: TrackLyrics | null) => Promise<void>;
}
...
let lastMetadataUpdate: {
  trackIds: string[];
  previousTracks: db.SavedTrack[];
  previousArtworks: Map<string, Blob | null>;
} | null = null;
...
export const useLocalLibraryStore = create<LocalLibraryState>((set, get) => ({
  ...
  loadSavedLibrary: async () => {
    try {
      ...
      for (const folder of folders) {
        if (folder.handle) {
          const status = await (folder.handle as any).queryPermission({ mode: 'read' });
          if (status !== 'granted') {
      ...
      const hydratedTracks: Track[] = await Promise.all(
        tracks.map(async (t) => {
  ...
  importDirectory: async () => {
    ...
    try {
      ...
      // Save folder handle
      const folder: LocalFolder = { id: folderId, name: folderName, handle };
      ...
      const discoveredFiles: Array<{ file: File; path: string; handle: FileSystemFileHandle }> = [];
      ...
      const audioFiles: Array<{ file: File; path: string; handle: FileSystemFileHandle }> = [];
      const lyricsFilesMap = new Map<string, { file: File; ext: string }>();
      ...
      const tracksToSave: db.SavedTrack[] = [];
      ...
      for (let i = 0; i < audioFiles.length; i++) {
        ...
        let trackLyrics: TrackLyrics | undefined = undefined;
  ...
  importFileList: async (files: FileList | File[]) => {
    try {
      ...
      const folder: LocalFolder = { id: folderId, name: folderName };
      await db.saveFolder(folder);
  ...
  resolveAudioFile: async (track: Track): Promise<File> => {
    ...
    if (!folder || !folder.handle) {
      throw new Error(
        'Audio file reference is unavailable. Please import or reselect the folder.'
      );
    }
    ...
    const status = await (folder.handle as any).queryPermission({ mode: 'read' });
    if (status !== 'granted') {
      const grantStatus = await (folder.handle as any).requestPermission({ mode: 'read' });
  ...
  saveTracksMetadata: async (
    ...
    try {
      ...
      const previousTracks: db.SavedTrack[] = [];
      const previousArtworks = new Map<string, Blob | null>();
      ...
      for (const id of trackIds) {
        ...
        if (fields.artist !== undefined && fields.artist.trim() !== '') updated.artist = fields.artist.trim();
        if (fields.album !== undefined && fields.album.trim() !== '') updated.album = fields.album.trim();
        if (fields.genre !== undefined && fields.genre.trim() !== '') updated.genre = fields.genre.trim();
        if (fields.year !== undefined) updated.year = fields.year;
        if (fields.albumArtist !== undefined) updated.albumArtist = fields.albumArtist.trim();

        if (trackIds.length === 1) {
          if (fields.title !== undefined && fields.title.trim() !== '') updated.title = fields.title.trim();
          if (fields.trackNumber !== undefined) updated.trackNumber = fields.trackNumber;
          if (fields.discNumber !== undefined) updated.discNumber = fields.discNumber;
          if (fields.composer !== undefined) updated.composer = fields.composer.trim();
          if (fields.comment !== undefined) updated.comment = fields.comment.trim();
        }
  ...
  saveTrackLyrics: async (trackId: string, lyrics: TrackLyrics | null) => {
    try {
      const dbTracks = await db.getTracks();
...
function splitGenres(genreStr: string): string[] {
  if (!genreStr) return [];
...
function formatGenreName(name: string): string {
  ...
  return cleaned
    .split(' ')
    .map(word => {
      if (word.toLowerCase() === 'lo-fi') return 'Lo-Fi';
      if (word.toLowerCase() === 'hi-fi') return 'Hi-Fi';
      return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
    })
    .join(' ');
}

function deriveCollections(tracks: Track[]): { albums: Album[]; artists: Artist[]; genres: GenreCollection[] } {
  ...
  for (const album of albums) {
    ...
    const discNumbers = albumTracksObjs.map(t => t.discNumber).filter((d): d is number => d !== undefined && d !== null);
  ...
  const artists = Array.from(artistMap.values()).map(art => {
    ...
    const appearsOnAlbums = albums.filter(al => {
      const albArt = (al.albumArtist || al.artistName || '').trim();
      const splitAlbArts = splitArtists(albArt).map(normalizeString);
      if (splitAlbArts.includes(normArt)) return false;
    ...
    const singlesAlbums = artistAlbums.filter(al => {
      if (al.tracks.length !== 1) return false;
  ...
  for (const gen of genres) {
    ...
    const genreArtists = artists.filter(art => {
      ...
      return tracks.some(t => {
        if (!gen.tracks.includes(t.id)) return false;
        const trackArts = splitArtists(t.artist || '').map(normalizeString);
        const albArts = t.albumArtist ? splitArtists(t.albumArtist).map(normalizeString) : [];
        return trackArts.includes(normArt) || albArts.includes(normArt);
      });
