...
export function BaseEmptyState({ icon, title, message, actionText, onAction }) {
    return React.createElement('div', { className: 'flex flex-col items-center justify-center text-center p-8 md:p-12 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[28px] max-w-lg mx-auto my-8 select-none' }, React.createElement('div', { className: 'w-16 h-16 rounded-full bg-glorify-accent/10 flex items-center justify-center text-glorify-accent mb-5 shadow-inner' }, icon), React.createElement('h3', { className: 'text-lg font-bold text-glorify-text-primary mb-2 tracking-tight' }, title), React.createElement('p', { className: 'text-xs text-glorify-text-muted leading-relaxed max-w-sm mb-6' }, message), actionText && onAction &&
