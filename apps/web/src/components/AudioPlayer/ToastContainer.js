...
export function ToastContainer() {
    ...
    return React.createElement('div', { className: 'fixed top-6 right-6 z-[10000] flex flex-col gap-3 pointer-events-none' }, React.createElement(AnimatePresence, null, toasts.map((toast) => {
        let icon = '🔔';
        if (toast.type === 'favorite')
            icon = '❤️';
        else if (toast.type === 'download')
            icon = '⬇️';
        else if (toast.type === 'queue')
            icon = '🎵';
        ...
        }, React.createElement('span', { className: 'text-sm' }, icon), React.createElement('span', null, toast.message), toast.action &&
