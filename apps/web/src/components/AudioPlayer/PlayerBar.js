...
export function PlayerBar({ isCollapsed = false, isDesktop = true }) {
    ...
    const handleQuickTabClick = (e, tab) => {
        e.stopPropagation();
    ...
    React.createElement('div', { className: 'flex items-center gap-ch-4 w-1/3 min-w-0 pl-ch-6 relative z-10' }, currentTrack.coverImage
        ? React.createElement(motion.img, {
            src: currentTrack.coverImage,
            alt: currentTrack.title,
            layoutId: 'player-artwork',
            className: `w-12 h-12 rounded-full object-cover border border-glorify-border-secondary/20 flex-shrink-0 shadow-lg animate-rotate-art ${isPlaying ? '' : 'animation-paused'} ${isPlaying ? 'album-pulse-playing' : ''}`,
        })
        : React.createElement('div', {
            className: `w-12 h-12 rounded-full bg-glorify-bg-secondary border border-glorify-border-secondary/20 flex items-center justify-center font-mono text-[9px] font-bold text-glorify-text-muted flex-shrink-0 shadow-lg animate-rotate-art ${isPlaying ? '' : 'animation-paused'}`,
        }, 'TRACK'), React.createElement('div', { className: 'flex-1 min-w-0 h-10 flex flex-col justify-center overflow-hidden relative' }, React.createElement(AnimatePresence, { mode: 'wait' }, React.createElement(motion.div, {
        key: currentTrack.id,
    ...
    }, React.createElement('span', { className: 'text-sm font-semibold text-glorify-text-primary truncate' }, currentTrack.title), React.createElement('span', { className: 'text-xs text-glorify-text-muted truncate mt-0.5 font-normal' }, currentTrack.artist)))), React.createElement(motion.button, {
        ...
        className: `p-2 rounded-full hover:bg-glorify-bg-secondary/60 outline-none focus-ring cursor-pointer transition-colors flex-shrink-0 ${isLiked ? 'text-glorify-accent' : 'text-glorify-text-muted hover:text-glorify-copper'}`,
    ...
    // Buttons Row
    React.createElement('div', { className: 'flex items-center gap-ch-5' }, 
    // Shuffle Button
    React.createElement(motion.button, {
        whileHover: { scale: 1.15 },
        onClick: toggleShuffle,
        className: `p-1.5 rounded-full cursor-pointer outline-none focus-ring relative transition-colors ${isShuffle ? 'text-glorify-accent' : 'text-glorify-text-muted hover:text-glorify-copper'}`,
    ...
    React.createElement(motion.button, {
        ...
        className: 'p-1.5 rounded-full text-glorify-text-secondary hover:text-glorify-copper cursor-pointer outline-none focus-ring transition-colors',
    ...
    React.createElement(motion.button, {
        ...
        whileTap: { scale: 0.92 },
        onClick: togglePlay,
        className: 'w-11 h-11 rounded-full bg-glorify-text-primary text-glorify-bg-primary flex items-center justify-center cursor-pointer outline-none focus-ring shadow-md hover:bg-glorify-copper hover:text-white transition-all',
    ...
    React.createElement(motion.button, {
        ...
        className: `p-1.5 rounded-full cursor-pointer outline-none focus-ring relative transition-colors ${repeatMode !== 'none' ? 'text-glorify-accent' : 'text-glorify-text-muted hover:text-glorify-copper'}`,
        'aria-label': `Repeat mode: ${repeatMode}`,
    }, React.createElement(Repeat, { className: 'w-ch-4.5 h-ch-4.5' }), repeatMode !== 'none' &&
        React.createElement('span', {
            className: 'absolute bottom-0 left-1/2 -translate-x-1/2 text-[8px] font-semibold leading-none',
        }, repeatMode === 'one' ? '1' : '•'))), React.createElement('div', { className: 'w-full flex items-center gap-ch-3 text-[10px] font-mono text-glorify-text-muted/80' }, React.createElement('span', { className: 'w-8 text-right' }, formatDuration(currentTime)), React.createElement('div', { className: 'relative w-full h-4 flex items-center group' }, 
    // Track Background
    ...
    React.createElement(motion.button, {
        whileHover: { scale: 1.15 },
        onClick: (e) => handleQuickTabClick(e, 'lyrics'),
    ...
    React.createElement(motion.button, {
        whileHover: { scale: 1.15 },
        onClick: (e) => handleQuickTabClick(e, 'queue'),
        className: 'p-1.5 rounded-full hover:bg-glorify-bg-secondary/60 text-glorify-text-muted hover:text-glorify-copper cursor-pointer outline-none focus-ring transition-colors',
        title: 'Queue',
    ...
    React.createElement(motion.button, {
        whileHover: { scale: 1.15 },
        onClick: (e) => handleQuickTabClick(e, 'settings'),
        className: 'p-1.5 rounded-full hover:bg-glorify-bg-secondary/60 text-glorify-text-muted hover:text-glorify-copper cursor-pointer outline-none focus-ring transition-colors',
        title: 'Devices',
    ...
    React.createElement(motion.div, {
        ...
        : React.createElement(Volume2, { className: 'w-ch-4.5 h-ch-4.5' })), React.createElement('input', {
        type: 'range',
        min: 0,
        max: 1,
        step: 0.02,
