...
import { Play, Pause, MoreHorizontal, Plus, Heart, Check, AlertCircle } from 'lucide-react';
...
export const TrackCard = React.memo(function TrackCard({
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        'div',
        { className: 'w-ch-6 flex items-center justify-center flex-shrink-0' },
        isCurrent && isPlaying
          ? React.createElement(
              'div',
              { className: 'group-hover:hidden flex items-end justify-center gap-[2px] w-4 h-4 pb-0.5' },
              React.createElement('div', { className: 'w-[2px] bg-glorify-accent eq-bar-1' }),
              React.createElement('div', { className: 'w-[2px] bg-glorify-accent eq-bar-2' }),
              React.createElement('div', { className: 'w-[2px] bg-glorify-accent eq-bar-3' })
            )
          : React.createElement(
              'span',
              {
                className: `text-xs font-mono group-hover:hidden ${
                  isCurrent ? 'text-glorify-accent font-semibold' : 'text-glorify-text-muted/70'
                }`,
              },
        ...
        React.createElement(
          'button',
          {
            ...
            className:
              'hidden group-hover:flex p-1 rounded-full text-glorify-text-primary hover:text-glorify-accent transition-colors outline-none focus-ring cursor-pointer border-none bg-transparent',
          },
          isCurrent && isPlaying
      ...
      React.createElement(
        ...
        React.createElement(
          'div',
          {
            className: `text-sm font-semibold truncate ${
              isCurrent ? 'text-glorify-accent' : 'text-glorify-text-primary'
            }`,
          },
          track.title
        ),
        React.createElement(
          'div',
          { className: 'text-xs text-glorify-text-muted truncate mt-0.5' },
          track.artist
        )
      )
    ),
    ...
    track.album &&
      React.createElement(
        'div',
        { className: 'hidden md:block flex-1 text-sm text-glorify-text-secondary truncate px-ch-4' },
        track.album
      ),
    ...
    React.createElement(
      'div',
      { className: 'flex items-center gap-ch-4 text-glorify-text-secondary flex-shrink-0' },
      
      // Simulated Download Indicators (offline state badge / progress)
      downloadState === 'downloading' &&
        React.createElement(
          'div',
          { className: 'flex items-center gap-1.5 text-[9px] font-mono text-glorify-accent' },
          React.createElement('span', { className: 'w-1.5 h-1.5 rounded-full bg-glorify-accent animate-ping' }),
          `${progressVal}%`
        ),
      ...
      React.createElement(
        motion.button,
        {
          ...
          whileTap: { scale: 0.8 },
          className: `p-1 rounded-full hover:bg-glorify-bg-secondary outline-none focus-ring cursor-pointer transition-all border-none bg-transparent ${
            isLiked 
      ...
      React.createElement(
        'span',
        { className: 'text-xs font-mono text-glorify-text-muted/80' },
