...
type LibraryTab = 'songs' | 'albums' | 'artists' | 'playlists' | 'liked' | 'downloads' | 'genres' | 'collections';
type SortCriteria = 'alphabetical' | 'recently_added' | 'creator' | 'duration' | 'year' | 'tracks_count';

export function Library() {
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  const rawTab = searchParams.get('tab') as LibraryTab;
  ...
  const [sortBy, setSortBy] = useState<SortCriteria>('alphabetical');
  
  // Layout states
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [cardSize, setCardSize] = useState<number>(170); // Minmax width slider
  ...
  const togglePinPlaylist = (e: React.MouseEvent, playlistId: string) => {
    e.stopPropagation();
  ...
  const processedSongs = useMemo(() => {
    ...
    else if (sortBy === 'creator') result.sort((a, b) => a.artist.localeCompare(b.artist));
  ...
  const processedAlbums = useMemo(() => {
    ...
    if (sortBy === 'alphabetical') {
      ...
      const getAlbumAddedTime = (al: Album) => {
      ...
      const getAlbumDuration = (al: Album) => {
        const albumTracks = combinedTracks.filter(t => al.tracks.includes(t.id));
  ...
  const processedPlaylists = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'creator') return a.userId.localeCompare(b.userId);
      if (sortBy === 'duration') return getPlaylistDuration(b) - getPlaylistDuration(a);
      return 0;
    });
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'flex items-center justify-between pb-ch-2 mt-ch-4' },
      React.createElement(
        'div',
        ...
        React.createElement(
          ...
          'Your personalized collection of playlists, followed artists, liked albums, and offline tracks.'
        )
      ),
      activeTab === 'playlists' &&
        React.createElement(
          'button',
          {
            ...
            className:
              'flex items-center gap-ch-2 px-ch-4 py-2 bg-glorify-accent text-glorify-carbon-950 rounded-full text-xs font-semibold shadow-md hover:scale-105 active:scale-95 transition-all cursor-pointer outline-none focus-ring',
          },
          React.createElement(Plus, { className: 'w-ch-4 h-ch-4' }),
          'Create Playlist'
        )
    ),
    ...
    React.createElement(
      'div',
      { className: 'flex flex-col gap-4 p-5 bg-glorify-bg-surface/50 border border-glorify-border-primary/10 rounded-2xl' },
      React.createElement(
        'div',
        ...
        React.createElement(
          'div',
          { className: 'flex flex-col gap-1' },
          React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary' }, 'LOCAL MUSIC'),
          folders.length > 0
            ? React.createElement(
                'p',
                { className: 'text-xs text-glorify-text-secondary font-medium' },
                `${localTracks.length} tracks • ${localAlbums.length} albums • ${localArtists.length} artists`
              )
            : React.createElement(
                'p',
                { className: 'text-xs text-glorify-text-muted font-normal' },
                'Import folders containing MP3, FLAC, M4A, etc. to organize them locally.'
              )
        ),
        React.createElement(
          'div',
      ...
      scanningState === 'scanning' &&
        React.createElement(
          'div',
          { className: 'flex flex-col gap-1.5 w-full mt-2' },
          React.createElement(
            'div',
            { className: 'flex justify-between text-[10px] text-glorify-text-secondary font-semibold' },
            React.createElement('span', null, 'Scanning local files...'),
            React.createElement('span', null, `${scanProgress.current} / ${scanProgress.total}`)
          ),
          React.createElement(
            'div',
            { className: 'w-full h-1.5 bg-glorify-bg-surface border border-glorify-border-primary/10 rounded-full overflow-hidden' },
      ...
      isPermissionRequired &&
        React.createElement(
          'div',
          { className: 'flex items-center justify-between p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl mt-2 text-xs text-amber-300 font-semibold' },
          React.createElement('span', null, 'Permission required to read files from folders.'),
          React.createElement(
            'button',
            {
    ...
    React.createElement(
      'div',
      { className: 'flex flex-col xl:flex-row xl:items-center justify-between gap-ch-4 pb-ch-2 mt-ch-1 border-b border-glorify-border-primary/5' },
      
      // Left: Tab chips
      React.createElement(
        'div',
        { className: 'flex items-center gap-ch-2 overflow-x-auto scrollbar-none py-1' },
        tabs.map((tab) =>
          React.createElement(
            'button',
            {
              ...
              className: `flex items-center gap-ch-1.5 px-ch-4 py-2 rounded-full text-xs font-semibold transition-all duration-300 cursor-pointer outline-none focus-ring ${
                activeTab === tab.id
            ...
            React.createElement(tab.icon, { className: 'w-ch-3.5 h-ch-3.5 flex-shrink-0' }),
      ...
      React.createElement(
        'div',
        { className: 'flex flex-wrap items-center gap-4 xl:justify-end' },
        
        // Dynamic Card Size Slider (Visible in Grid View)
        viewMode === 'grid' && activeTab !== 'songs' && activeTab !== 'downloads' && activeTab !== 'collections' &&
          React.createElement(
            'div',
            { className: 'flex items-center gap-2 bg-glorify-bg-surface/40 border border-glorify-border-primary/10 rounded-full px-4 py-1.5 shadow-sm text-xs text-glorify-text-secondary font-medium' },
            React.createElement(SlidersHorizontal, { className: 'w-3.5 h-3.5 text-glorify-accent' }),
            React.createElement('span', null, 'Card Size'),
            React.createElement('input', {
              type: 'range',
              min: 120,
              max: 240,
        ...
        activeTab !== 'songs' && activeTab !== 'downloads' &&
          React.createElement(
            'div',
            { className: 'flex items-center bg-glorify-bg-surface/45 border border-glorify-border-primary/10 rounded-full p-0.5 shadow-sm' },
            React.createElement(
              'button',
              {
              ...
              React.createElement(LayoutGrid, { className: 'w-3.5 h-3.5' })
            ),
            React.createElement(
              'button',
              {
        ...
        React.createElement(
          ...
          activeTab === 'albums'
            ? React.createElement(
                React.Fragment,
                null,
                React.createElement('option', { value: 'alphabetical' }, 'Album Name'),
                React.createElement('option', { value: 'creator' }, 'Artist'),
                React.createElement('option', { value: 'year' }, 'Release Year'),
                React.createElement('option', { value: 'recently_added' }, 'Recently Added'),
                React.createElement('option', { value: 'tracks_count' }, 'Track Count'),
            ...
            : React.createElement(
                ...
                React.createElement('option', { value: 'recently_added' }, 'Recently Added'),
                React.createElement('option', { value: 'creator' }, 'Creator'),
                React.createElement('option', { value: 'duration' }, 'Duration')
              )
        ),
        ...
        activeTab === 'songs' && localTracks.length > 0 &&
          React.createElement(
            'button',
            {
              ...
              className: `px-4 py-1.5 rounded-full border border-glorify-border-primary/10 text-xs font-semibold cursor-pointer transition-all shadow-sm ${
                selectionMode ? 'bg-glorify-accent text-glorify-carbon-950 font-bold' : 'bg-glorify-bg-surface/45 text-glorify-text-primary hover:text-glorify-accent'
              }`
            },
    ...
    React.createElement(
      'div',
      { className: 'flex-1 min-h-[50vh] mt-ch-4' },
      React.createElement(
        AnimatePresence,
        { mode: 'wait' },
        React.createElement(
          ...
          activeTab === 'songs' &&
            React.createElement(
              'div',
              { className: 'flex flex-col gap-1 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[20px] p-ch-2 shadow-sm' },
              processedSongs.length === 0
                ...
                : processedSongs.map((track, idx) =>
                    React.createElement(
                      'div',
                      { key: track.id, className: 'flex items-center gap-3 w-full pr-2' },
                      selectionMode && track.source === 'local' &&
                        React.createElement('input', {
                          type: 'checkbox',
          ...
          activeTab === 'albums' &&
            (combinedAlbums.length === 0
              ? React.createElement(
                  'div',
                  { className: 'flex flex-col items-center justify-center text-center py-20 px-6 bg-glorify-bg-surface/10 border border-glorify-border-primary/5 rounded-[28px] gap-4 max-w-sm mx-auto font-sans shadow-sm' },
                  React.createElement(Disc, { className: 'w-12 h-12 text-glorify-accent animate-pulse' }),
                  React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary' }, 'No albums yet'),
                  React.createElement(
                    'p',
                    { className: 'text-xs text-glorify-text-muted leading-relaxed' },
                    'Import your music library to start building your collection.'
                  ),
                  React.createElement(
                    'button',
                    {
              ...
              : (viewMode === 'grid'
                  ? React.createElement(
                      'div',
                      { style: gridStyles, className: 'grid gap-ch-6' },
                  ...
                  : React.createElement(
                      'div',
                      { className: 'flex flex-col gap-1.5 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[20px] p-4 shadow-sm font-sans' },
                      React.createElement(
                        'div',
                        { className: 'grid grid-cols-12 items-center px-3 py-2 text-[10px] font-bold text-glorify-text-muted uppercase tracking-wider border-b border-glorify-border-primary/5 mb-1' },
                        React.createElement('div', { className: 'col-span-5 md:col-span-4' }, 'Album'),
                        React.createElement('div', { className: 'col-span-3 md:col-span-3' }, 'Artist'),
                        React.createElement('div', { className: 'col-span-2 md:col-span-2' }, 'Year'),
                        React.createElement('div', { className: 'hidden sm:block sm:col-span-1' }, 'Tracks'),
                        React.createElement('div', { className: 'col-span-2 md:col-span-2 text-right pr-2' }, 'Duration')
                      ),
                      processedAlbums.map((album) => {
                        ...
                        return React.createElement(
                          ...
                          React.createElement(
                            ...
                            React.createElement('img', { src: album.coverUrl || 'https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=100', className: 'w-10 h-10 rounded object-cover flex-shrink-0' }),
                            React.createElement('div', { className: 'text-sm font-semibold text-glorify-text-primary truncate' }, album.title)
                          ),
                          React.createElement(
                            'div',
                            { className: 'col-span-3 md:col-span-3 text-xs text-glorify-text-secondary truncate' },
                            album.albumArtist || album.artistName
                          ),
                          React.createElement(
                            'div',
                            { className: 'col-span-2 md:col-span-2 text-xs text-glorify-text-secondary' },
                            album.releaseYear || '-'
                          ),
                          React.createElement(
                            'div',
                            { className: 'hidden sm:block sm:col-span-1 text-xs text-glorify-text-muted' },
                            `${songCount} song${songCount === 1 ? '' : 's'}`
                          ),
                          React.createElement(
                            'div',
                            { className: 'col-span-2 md:col-span-2 text-right text-xs text-glorify-text-muted pr-2' },
                            durationText
                          )
                        );
                      })
          ...
          activeTab === 'artists' &&
            (viewMode === 'grid'
              ...
              : React.createElement(
                  'div',
                  { className: 'flex flex-col gap-1.5 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[20px] p-4 shadow-sm' },
                  processedArtists.map((artist) =>
                    React.createElement(
                      'div',
                      {
                      ...
                      React.createElement(
                        ...
                        React.createElement('img', { src: artist.avatarUrl, className: 'w-10 h-10 rounded-full object-cover flex-shrink-0' }),
                        React.createElement(
                          'div',
                          { className: 'min-w-0' },
                          React.createElement('div', { className: 'text-sm font-semibold text-glorify-text-primary truncate' }, artist.name),
                          React.createElement('div', { className: 'text-xs text-glorify-text-muted mt-0.5' }, artist.genres.join(' / '))
                        )
                      )
                    )
                  )
          ...
          activeTab === 'genres' &&
            (localGenres.length === 0
              ? React.createElement(
                  'div',
                  { className: 'flex flex-col items-center justify-center text-center py-20 px-6 bg-glorify-bg-surface/10 border border-glorify-border-primary/5 rounded-[28px] gap-4 max-w-sm mx-auto font-sans shadow-sm' },
                  React.createElement(Tag, { className: 'w-12 h-12 text-glorify-accent animate-pulse' }),
                  React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary' }, 'No genres yet'),
                  React.createElement(
                    'p',
                    { className: 'text-xs text-glorify-text-muted leading-relaxed' },
                    'Import music files containing genre tags to populate this section.'
                  )
                )
              : React.createElement(
                  'div',
              ...
              : (viewMode === 'grid'
                  ? React.createElement(
                      ...
                      processedPlaylists.map((playlist) => {
                        ...
                        return React.createElement(
                          'div',
                          { key: playlist.id, className: 'relative group' },
                          React.createElement(CatalogCard, {
                            id: playlist.id,
                            title: playlist.name,
                          ...
                          React.createElement(
                            'button',
                            {
                              ...
                              className: `absolute top-3 right-3 p-1.5 rounded-full cursor-pointer outline-none transition-all shadow ${
                                isPinned
                      ...
                      processedPlaylists.map((playlist) => {
                        ...
                        return React.createElement(
                          ...
                          React.createElement(
                            'div',
                            { className: 'flex items-center gap-3 min-w-0' },
                            React.createElement('img', { src: playlist.coverImage || 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=100', className: 'w-10 h-10 rounded object-cover flex-shrink-0' }),
                            React.createElement(
                              'div',
                              { className: 'min-w-0' },
                              React.createElement(
                                'div',
                                { className: 'text-sm font-semibold text-glorify-text-primary truncate flex items-center gap-1.5' },
                                playlist.name,
                                isPinned && React.createElement(Pin, { className: 'w-3 h-3 text-glorify-accent fill-currentColor flex-shrink-0' })
                              ),
                              React.createElement('div', { className: 'text-xs text-glorify-text-muted mt-0.5' }, `${playlist.songs.length} song${playlist.songs.length === 1 ? '' : 's'}`)
                            )
                          ),
                          React.createElement(
                            'button',
            ...
            React.createElement(
              ...
              processedCollections.length === 0
                ...
                : processedCollections.map((collection) => {
                    ...
                    const handleShuffleCollection = (e: React.MouseEvent) => {
                      e.stopPropagation();
                      if (collection.tracks.length > 0) {
                    ...
                    return React.createElement(
                      ...
                      React.createElement(
                        ...
                        coverUrl
                          ...
                          : React.createElement(
                              'div',
                              { className: `w-full h-full bg-gradient-to-br ${collection.gradient} flex flex-col items-center justify-center gap-2 p-4 text-center` },
                              React.createElement(collection.icon, { className: 'w-10 h-10 text-white/90 drop-shadow-md' })
                            ),
                        React.createElement('div', {
                          ...
                          trackCount > 0 && React.createElement(
                            ...
                            React.createElement(Play, { className: 'w-4 h-4 fill-currentColor pl-0.5' })
                          ),
                          trackCount > 0 && React.createElement(
                            'button',
                            {
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          'p',
                          { className: 'text-xs text-glorify-text-muted truncate mb-2' },
                          collection.description
                        ),
                        React.createElement(
                          'div',
                          { className: 'text-[10px] uppercase tracking-wider text-glorify-accent font-bold' },
            ...
            (favoritedTracks.length === 0 && processedAlbums.length === 0 && processedArtists.length === 0
              ...
              : React.createElement(
                  ...
                  React.createElement(
                    ...
                    React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Liked Songs'),
                    React.createElement(
                      'div',
                      { className: 'flex flex-col bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[22px] p-2 shadow-sm' },
                  ...
                  React.createElement(
                    'div',
                    { className: 'flex flex-col gap-ch-3' },
                    React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Liked Albums'),
                    processedAlbums.length === 0
                              ...
                              processedAlbums.map((album) =>
                                React.createElement(
                                  ...
                                  React.createElement(
                                    'div',
                                    { className: 'flex items-center gap-3 min-w-0' },
                                    React.createElement('img', { src: album.coverUrl, className: 'w-10 h-10 rounded object-cover flex-shrink-0' }),
                                    React.createElement(
                                      'div',
                                      { className: 'min-w-0' },
                                      React.createElement('div', { className: 'text-sm font-semibold text-glorify-text-primary truncate' }, album.title),
                                      React.createElement('div', { className: 'text-xs text-glorify-text-muted mt-0.5' }, album.artistName)
                                    )
                                  )
                                )
                              )
                  ...
                  React.createElement(
                    'div',
                    { className: 'flex flex-col gap-ch-3' },
                    React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Followed Artists'),
                    processedArtists.length === 0
                              ...
                              processedArtists.map((art) =>
                                React.createElement(
                                  ...
                                  React.createElement(
                                    'div',
                                    { className: 'flex items-center gap-3 min-w-0' },
                                    React.createElement('img', { src: art.avatarUrl, className: 'w-10 h-10 rounded-full object-cover flex-shrink-0' }),
                                    React.createElement(
                                      'div',
                                      { className: 'min-w-0' },
                                      React.createElement('div', { className: 'text-sm font-semibold text-glorify-text-primary truncate' }, art.name),
                                      React.createElement('div', { className: 'text-xs text-glorify-text-muted mt-0.5' }, art.genres.join(' / '))
                                    )
                                  )
                                )
                              )
          ...
          activeTab === 'downloads' &&
            (downloadedTracks.length === 0
              ...
              : React.createElement(
                  'div',
                  { className: 'flex flex-col gap-1 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[22px] p-2 shadow-sm' },
    ...
    selectionMode && selectedTrackIds.length > 0 &&
      React.createElement(
        'div',
        { className: 'fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-glorify-bg-surface/95 border border-glorify-border-primary/20 backdrop-blur-md rounded-2xl px-6 py-4 shadow-2xl flex items-center justify-between gap-8 text-xs font-bold text-glorify-text-primary min-w-[320px] pointer-events-auto' },
        React.createElement('span', null, `${selectedTrackIds.length} tracks selected`),
        React.createElement(
          'div',
          { className: 'flex items-center gap-3' },
          React.createElement(
