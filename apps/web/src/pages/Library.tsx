...
type Tab = 'songs' | 'albums' | 'artists' | 'favorites';
...
export function Library() {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'flex items-center justify-between border-b border-chotify-border-primary pb-ch-4' },
      React.createElement(
        'div',
        { className: 'flex flex-col gap-ch-1' },
        React.createElement(
          'h1',
          { className: 'text-2xl lg:text-3xl font-bold tracking-tight text-chotify-text-primary' },
          'USER LIBRARY'
        ),
        React.createElement(
          'p',
          { className: 'text-sm text-chotify-text-muted font-mono uppercase tracking-wider' },
          'Browse and play your custom audio collection'
        )
      ),
      ...
      activeTab === 'favorites' &&
        React.createElement(
          'button',
          {
            ...
            className:
              'flex items-center gap-ch-2 px-ch-4 py-ch-2 bg-chotify-text-primary text-chotify-bg-primary rounded-ch-sm text-xs font-bold shadow-sm hover:shadow-ch-glow cursor-pointer active:scale-95 transition-all outline-none',
          },
    ...
    React.createElement(
      'div',
      { className: 'flex items-center gap-ch-3 border-b border-chotify-border-primary pb-ch-2 overflow-x-auto scrollbar-none' },
      tabs.map((tab) =>
        React.createElement(
          'button',
          {
            ...
            className: `flex items-center gap-ch-2 px-ch-4 py-ch-2.5 rounded-ch-sm text-xs font-mono tracking-wider transition-colors cursor-pointer outline-none focus-ring ${
              activeTab === tab.id
                ? 'bg-chotify-bg-secondary text-chotify-text-primary font-bold'
                : 'text-chotify-text-muted hover:text-chotify-text-secondary'
            }`,
          },
          React.createElement(tab.icon, { className: 'w-ch-4 h-ch-4' }),
      ...
      activeTab === 'albums' &&
        React.createElement(
          ...
          albums.length === 0
            ? React.createElement(
                'div',
                { className: 'col-span-full text-center py-12 text-sm font-mono text-chotify-text-muted' },
                'NO_ALBUMS_FOUND'
              )
            : albums.map((album) =>
