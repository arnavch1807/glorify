...
export function GenrePage() {
    ...
    const { playTrack, setQueue, toggleShuffle, totalPlays } = usePlayerStore();
    ...
    if (!genreObj) {
        return React.createElement('div', { className: 'text-center py-16 font-sans' }, React.createElement('h2', { className: 'text-xl font-bold text-glorify-text-primary' }, 'Genre Not Found'), React.createElement('button', { onClick: () => navigate(-1), className: 'mt-ch-4 text-xs text-glorify-accent hover:underline' }, 'Go Back'));
    }
    ...
    }, React.createElement('div', { className: 'flex flex-col md:flex-row items-center md:items-end gap-ch-6 text-center md:text-left z-10' }, React.createElement('div', { className: 'w-32 h-32 md:w-36 md:h-36 rounded-2xl overflow-hidden border-2 border-glorify-accent/30 shadow-2xl bg-gradient-to-br from-glorify-carbon-900 to-glorify-carbon-950 flex items-center justify-center flex-shrink-0' }, React.createElement(Tag, { className: 'w-16 h-16 text-glorify-accent' })), React.createElement('div', { className: 'flex flex-col gap-ch-2 mt-ch-4 md:mt-0' }, React.createElement('span', { className: 'text-[10px] font-bold text-glorify-accent tracking-widest uppercase' }, 'GENRE COLLECTION'), React.createElement('h1', { className: 'text-4xl md:text-5xl font-extrabold tracking-tight text-glorify-text-primary leading-none' }, genreObj.name), React.createElement('div', { className: 'flex items-center justify-center md:justify-start gap-2 text-xs text-glorify-text-secondary font-medium mt-1' }, React.createElement('span', { className: 'text-glorify-text-primary font-semibold' }, `${genreTracks.length} song${genreTracks.length === 1 ? '' : 's'} · ${genreAlbums.length} album${genreAlbums.length === 1 ? '' : 's'} · ${genreArtists.length} artist${genreArtists.length === 1 ? '' : 's'}`))))), 
    // Controls subheader toolbar
    React.createElement('div', { className: 'sticky top-0 z-20 py-4 bg-glorify-bg-primary/95 backdrop-blur-md border-b border-glorify-border-primary/5 flex items-center justify-between flex-wrap gap-ch-3' }, React.createElement('div', { className: 'flex items-center gap-ch-3' }, genreTracks.length > 0 &&
        ...
        }, React.createElement(Play, { className: 'w-5 h-5 fill-currentColor pl-0.5' }), 'Play All'), genreTracks.length > 0 &&
        React.createElement(motion.button, {
    ...
    React.createElement('div', { className: 'grid grid-cols-1 lg:grid-cols-2 gap-10 z-10' }, 
    // Column Left: Popular Compositions
    React.createElement('div', { className: 'flex flex-col gap-ch-4 text-left' }, React.createElement('div', { className: 'flex items-center gap-2 pl-ch-1 text-glorify-text-primary' }, React.createElement(Sparkles, { className: 'w-4 h-4 text-glorify-accent' }), React.createElement('h2', { className: 'text-lg font-bold' }, 'Popular Compositions')), React.createElement('div', { className: 'flex flex-col bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[24px] p-ch-3 shadow-sm' }, popularTracks.length === 0
        ? React.createElement('div', { className: 'text-center py-10 text-xs text-glorify-text-muted' }, 'No popular songs')
        : popularTracks.map((track, idx) => React.createElement(TrackCard, {
            key: 'pop-' + track.id,
            track: track,
            index: idx,
    ...
    React.createElement('div', { className: 'flex flex-col gap-ch-4 text-left' }, React.createElement('div', { className: 'flex items-center gap-2 pl-ch-1 text-glorify-text-primary' }, React.createElement(Clock, { className: 'w-4 h-4 text-glorify-accent' }), React.createElement('h2', { className: 'text-lg font-bold' }, 'Recently Added')), React.createElement('div', { className: 'flex flex-col bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[24px] p-ch-3 shadow-sm' }, recentlyAddedTracks.length === 0
        ? React.createElement('div', { className: 'text-center py-10 text-xs text-glorify-text-muted' }, 'No tracks imported yet')
        : recentlyAddedTracks.map((track, idx) => React.createElement(TrackCard, {
            key: 'recent-' + track.id,
            track: track,
            index: idx,
            queueContext: genreTracks,
    ...
    genreAlbums.length > 0 &&
        React.createElement('div', { className: 'flex flex-col gap-ch-4 z-10' }, React.createElement('h2', { className: 'text-lg font-bold text-glorify-text-primary pl-ch-1 text-left' }, 'Albums'), React.createElement('div', { className: 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-ch-6' }, genreAlbums.map((album) => React.createElement(CatalogCard, {
            key: album.id,
            id: album.id,
            title: album.title,
    ...
    genreArtists.length > 0 &&
        React.createElement('div', { className: 'flex flex-col gap-ch-4 z-10' }, React.createElement('h2', { className: 'text-lg font-bold text-glorify-text-primary pl-ch-1 text-left' }, 'Artists'), React.createElement('div', { className: 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-ch-6' }, genreArtists.map((artist) => React.createElement(CatalogCard, {
            key: artist.id,
            id: artist.id,
            title: artist.name,
