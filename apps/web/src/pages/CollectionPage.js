...
import { Play, Shuffle, ArrowLeft, Music, Sparkles, Heart, Disc, ListMusic, SlidersHorizontal, Search } from 'lucide-react';
export function CollectionPage() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [allTracks, setAllTracks] = useState([]);
    const [filterQuery, setFilterQuery] = useState('');
    const { playTrack, setQueue, toggleShuffle, isShuffle, favoritedTrackIds, listeningHistory, totalPlays } = usePlayerStore();
    ...
    const filteredTracks = useMemo(() => {
        ...
        return collection.tracks.filter(t => t.title.toLowerCase().includes(q) ||
    ...
    if (!collection) {
        return React.createElement('div', { className: 'flex flex-col items-center justify-center h-[50vh] gap-4 font-sans text-center' }, React.createElement('div', { className: 'text-sm text-glorify-text-muted' }, 'Smart collection not found.'), React.createElement('button', { onClick: () => navigate('/library?tab=collections'), className: 'px-4 py-2 bg-glorify-accent text-glorify-carbon-950 font-bold rounded-full text-xs cursor-pointer' }, 'Back to Library'));
    }
    ...
    return React.createElement('div', { className: 'flex flex-col gap-6 w-full mx-auto pb-32 text-left font-sans' }, 
    // Back Button
    ...
    }, React.createElement(ArrowLeft, { className: 'w-ch-4 h-ch-4' }), 'Back to Library'), 
    // Header Jumbotron
    ...
    // Cover Artwork
    React.createElement('div', { className: 'w-40 h-40 md:w-44 md:h-44 rounded-[24px] shadow-2xl flex-shrink-0 overflow-hidden bg-glorify-bg-secondary flex items-center justify-center' }, coverUrl
        ? React.createElement('img', { src: coverUrl, alt: collection.name, className: 'w-full h-full object-cover' })
        : React.createElement('div', { className: `w-full h-full bg-gradient-to-br ${collection.gradient} flex items-center justify-center p-4` }, React.createElement(collection.icon, { className: 'w-16 h-16 text-white/90 drop-shadow-lg' }))), 
    // Metadata Info
    React.createElement('div', { className: 'flex flex-col gap-1.5 md:mb-1' }, React.createElement('span', { className: 'text-[10px] uppercase font-bold tracking-widest text-glorify-accent' }, 'Smart Collection'), React.createElement('h1', { className: 'text-2xl md:text-3xl font-extrabold text-glorify-text-primary tracking-tight' }, collection.name), React.createElement('p', { className: 'text-xs text-glorify-text-muted leading-relaxed max-w-md' }, collection.description), React.createElement('div', { className: 'flex items-center justify-center md:justify-start gap-1.5 text-xs text-glorify-text-secondary mt-1 font-semibold' }, React.createElement('span', null, `${trackCount} song${trackCount === 1 ? '' : 's'}`), React.createElement('span', null, '•'), React.createElement('span', null, formatTotalTime(totalDuration))))), 
    // Playback controls panel
    React.createElement('div', { className: 'flex flex-wrap items-center justify-between gap-4 mt-2' }, React.createElement('div', { className: 'flex items-center gap-3' }, React.createElement('button', {
    ...
    }, React.createElement(Play, { className: 'w-ch-4 h-ch-4 fill-currentColor pl-0.5' }), 'Play All'), React.createElement('button', {
        disabled: trackCount === 0,
    ...
    }, React.createElement(Shuffle, { className: 'w-ch-3.5 h-ch-3.5' }), 'Shuffle')), 
    // Filter Search Box
    trackCount > 0 && React.createElement('div', { className: 'relative max-w-xs w-full' }, React.createElement(Search, { className: 'absolute left-ch-3 top-1/2 -translate-y-1/2 w-ch-3.5 h-ch-3.5 text-glorify-text-muted' }), React.createElement('input', {
        type: 'text',
    ...
    React.createElement('div', { className: 'flex flex-col gap-1 bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[24px] p-ch-3 mt-4 shadow-sm' }, trackCount === 0
        ? React.createElement('div', { className: 'flex flex-col items-center justify-center text-center py-20 px-6 gap-3' }, React.createElement(collection.icon, { className: 'w-12 h-12 text-glorify-accent animate-pulse' }), React.createElement('h3', { className: 'text-sm font-bold text-glorify-text-primary' }, 'Collection is empty'), React.createElement('p', { className: 'text-xs text-glorify-text-muted max-w-xs leading-relaxed' }, collection.emptyState))
        : (filteredTracks.length === 0
            ...
            : filteredTracks.map((track, idx) => React.createElement(TrackCard, {
                key: track.id,
                track: track,
                index: idx,
