...
import { Search as SearchIcon, Clock, Trash2, ArrowRight, X, Flame, Disc, Play, SlidersHorizontal } from 'lucide-react';
...
export function Search() {
  ...
  const [sourceFilter, setSourceFilter] = useState<'all' | 'standard' | 'ai'>('all');
  ...
  const filteredResults = useMemo(() => {
    ...
    return {
      tracks: combinedTracks.filter(t => {
        ...
        // Source Filter
        if (sourceFilter === 'standard' && t.isGenerated) return false;
        if (sourceFilter === 'ai' && !t.isGenerated) return false;

        // BPM Filter
        if (bpmFilter === 'slow' && (t.bpm === undefined || t.bpm >= 90)) return false;
        if (bpmFilter === 'medium' && (t.bpm === undefined || t.bpm < 90 || t.bpm > 120)) return false;
        if (bpmFilter === 'fast' && (t.bpm === undefined || t.bpm <= 120)) return false;

        // Key Signature Filter
        if (keyFilter !== 'all' && t.keySignature !== keyFilter) return false;
  ...
  const topResult = useMemo(() => {
    ...
    if (filteredResults.tracks.length > 0) {
      ...
      return { id: t.id, title: t.title, subtitle: t.artist, coverUrl: t.coverImage, type: 'track' as const, raw: t };
    }
    if (filteredResults.artists.length > 0) {
      const ar = filteredResults.artists[0];
      return { id: ar.id, title: ar.name, subtitle: 'Artist', coverUrl: ar.avatarUrl, type: 'artist' as const, raw: ar };
    }
    if (filteredResults.albums.length > 0) {
      const al = filteredResults.albums[0];
      return { id: al.id, title: al.title, subtitle: al.artistName, coverUrl: al.coverUrl, type: 'album' as const, raw: al };
    }
    return null;
  }, [filteredResults, debouncedQuery]);

  const flatResultsList = useMemo(() => {
    const list: Array<RecentSearchItem> = [];
  ...
  const triggerAction = (item: RecentSearchItem) => {
  ...
  const categoriesList: Array<{ id: SearchCategory; label: string }> = [
  ...
  return React.createElement(
    'div',
    { className: 'flex flex-col gap-8 w-full mx-auto mt-4 pb-32 font-sans' },
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('input', {
            ...
            className:
              'w-full h-14 pl-ch-14 pr-ch-12 rounded-full bg-glorify-bg-surface/50 border border-glorify-border-primary/10 text-base text-glorify-text-primary placeholder-glorify-text-muted focus:border-glorify-accent/40 outline-none transition-all duration-300 shadow-sm focus:shadow-md focus-ring',
        ...
        React.createElement(
          'button',
          {
            ...
            className: `px-6 h-14 rounded-full border text-xs font-semibold flex items-center gap-2 shadow-sm focus-ring cursor-pointer transition-all duration-300 ${
              showFilters || sourceFilter !== 'all' || bpmFilter !== 'all' || keyFilter !== 'all'
          ...
          React.createElement(SlidersHorizontal, { className: 'w-4 h-4' }),
          'Filters'
        )
      ),
      ...
      showFilters &&
        React.createElement(
          'div',
          { className: 'flex flex-wrap gap-6 p-5 bg-glorify-bg-surface/50 border border-glorify-border-primary/10 rounded-[22px] text-left shadow-sm font-sans animate-fade-in' },
          // Source filter
          React.createElement(
            'div',
            { className: 'flex flex-col gap-1.5' },
            React.createElement('span', { className: 'text-[10px] font-bold text-glorify-text-secondary tracking-widest uppercase pl-1' }, 'Source'),
            React.createElement(
              'div',
              { className: 'flex items-center bg-glorify-bg-secondary/60 border border-glorify-border-primary/5 p-1 rounded-full' },
              [
                { id: 'all', label: 'All' },
                ...
                React.createElement(
                  'button',
                  {
                    ...
                    className: `px-4 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer outline-none ${
                      sourceFilter === opt.id
          ...
          React.createElement(
            'div',
            { className: 'flex flex-col gap-1.5' },
            React.createElement('span', { className: 'text-[10px] font-bold text-glorify-text-secondary tracking-widest uppercase pl-1' }, 'Tempo (BPM)'),
            React.createElement(
              'div',
              { className: 'flex items-center bg-glorify-bg-secondary/60 border border-glorify-border-primary/5 p-1 rounded-full' },
              [
                ...
                React.createElement(
                  ...
                  {
                    key: opt.id,
                    onClick: () => setBpmFilter(opt.id as any),
                    className: `px-4 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer outline-none ${
                      bpmFilter === opt.id
                        ? 'bg-glorify-bg-surface text-glorify-text-primary font-bold shadow-sm'
                        : 'text-glorify-text-muted hover:text-glorify-text-primary'
          ...
          React.createElement(
            ...
            React.createElement('span', { className: 'text-[10px] font-bold text-glorify-text-secondary tracking-widest uppercase pl-1' }, 'Key Signature'),
            React.createElement(
              'select',
              {
                value: keyFilter,
      ...
      query.trim() &&
        React.createElement(
          'div',
          { className: 'flex items-center gap-ch-2 overflow-x-auto scrollbar-none pb-1' },
          categoriesList.map((cat) =>
            React.createElement(
              'button',
              {
                ...
                className: `px-ch-4 py-2 rounded-full text-xs font-semibold cursor-pointer outline-none transition-all focus-ring ${
        ...
        ? React.createElement(
            ...
            activeCategory === 'all' && topResult
              ? React.createElement(
                  ...
                  React.createElement(
                    'div',
                    { className: 'lg:col-span-2 flex flex-col gap-4 text-left' },
                    React.createElement('span', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Top Result'),
                    React.createElement(
                      motion.div,
                      {
                      ...
                      React.createElement(
                        'div',
                        { className: 'flex flex-col gap-1 pr-16' },
                        React.createElement('h2', { className: 'text-2xl font-extrabold text-glorify-text-primary leading-tight truncate' }, topResult.title),
                        React.createElement(
                          'div',
                          { className: 'flex items-center gap-2 mt-1' },
                          React.createElement('span', { className: 'text-[9px] font-mono font-bold tracking-widest text-glorify-accent bg-glorify-accent-glow px-2 py-0.5 rounded-sm' }, topResult.type.toUpperCase()),
                          React.createElement('span', { className: 'text-xs text-glorify-text-secondary truncate' }, topResult.subtitle)
                        )
                      ),
                  ...
                  React.createElement(
                    ...
                    React.createElement('span', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Songs'),
                    React.createElement(
                      'div',
                      { className: 'flex flex-col bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[28px] p-2 shadow-sm' },
                      filteredResults.tracks.slice(0, 4).map((track, idx) =>
            ...
            flatResultsList.length === 0
              ...
              : (activeCategory === 'songs'
                  ? React.createElement(
                      'div',
                      { className: 'flex flex-col bg-glorify-bg-surface/20 border border-glorify-border-primary/5 rounded-[28px] p-2 shadow-sm text-left' },
                  ...
                  : React.createElement(
                      'div',
                      { className: 'flex flex-col gap-3 bg-glorify-bg-surface/20 border border-glorify-border-primary/10 p-2 rounded-[24px]' },
                      flatResultsList.map((item, idx) =>
                        React.createElement(
                          'div',
                          {
                            key: item.id + '-' + item.type + '-' + idx,
                            onClick: () => triggerAction(item),
                            className: `flex items-center justify-between p-3.5 rounded-[16px] transition-all duration-200 cursor-pointer border border-transparent ${
                              focusedIndex === idx
                          ...
                          React.createElement(
                            ...
                            React.createElement(
                              'span',
                              { className: 'text-[9px] font-mono tracking-widest text-glorify-accent bg-glorify-accent-glow px-2 py-0.5 rounded-sm flex-shrink-0 font-semibold' },
                              item.type === 'track' ? 'SONG' : item.type.toUpperCase()
                            ),
                            React.createElement(
                              'div',
                              { className: 'min-w-0' },
                              React.createElement('div', { className: 'text-sm font-semibold truncate' }, highlightText(item.title, query)),
                              React.createElement('div', { className: 'text-xs text-glorify-text-muted truncate mt-0.5' }, highlightText(item.subtitle, query))
                            )
                          ),
                          React.createElement(ArrowRight, { className: 'w-ch-4.5 h-ch-4.5 text-glorify-text-muted opacity-40 flex-shrink-0' })
                        )
                      )
                    )
                )
        ...
        : React.createElement(
            ...
            recentSearches.length > 0 &&
              React.createElement(
                ...
                React.createElement(
                  'div',
                  { className: 'flex items-center justify-between pl-ch-1' },
                  React.createElement('span', { className: 'text-sm font-bold text-glorify-text-primary' }, 'Recently Searched'),
                  React.createElement(
                    'button',
                    { onClick: clearHistory, className: 'text-xs text-glorify-error hover:underline flex items-center gap-1 cursor-pointer outline-none border-none bg-transparent' },
                    React.createElement(Trash2, { className: 'w-ch-3.5 h-ch-3.5' }),
                    'Clear History'
                  )
                ),
                React.createElement(
                  'div',
                  { className: 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-ch-6' },
            ...
            React.createElement(
              'div',
              { className: 'flex flex-col gap-ch-4' },
              React.createElement('span', { className: 'text-sm font-bold text-glorify-text-primary pl-ch-1' }, 'Trending Genres'),
              React.createElement(
                'div',
                { className: 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-ch-6' },
                [
                  ...
                  React.createElement(
                    ...
                    React.createElement('span', { className: 'text-sm font-bold text-white leading-tight pr-4 drop-shadow' }, genre.name),
                    React.createElement(Disc, { className: 'absolute -right-4 -bottom-4 w-20 h-20 text-white/10 group-hover:rotate-45 transition-transform duration-700' })
                  )
                )
              )
            )
