...
import { Search, Flame, Music, Sparkles, Disc, Users, Clock, Trash2, ArrowRight } from 'lucide-react';
...
export function Explore() {
  ...
  const filteredResults = useMemo(() => {
    ...
    const matchTracks = tracks.filter(
      (t) =>
    ...
    const matchAlbums = albums.filter(
      (al) =>
        al.title.toLowerCase().includes(searchTerm) ||
        al.artistName.toLowerCase().includes(searchTerm)
    );

    const matchArtists = artists.filter(
      (ar) =>
        ar.name.toLowerCase().includes(searchTerm) ||
        ar.genres.some((g) => g.toLowerCase().includes(searchTerm))
    );

    const matchPlaylists = playlists.filter(
      (pl) =>
        pl.name.toLowerCase().includes(searchTerm) ||
  ...
  const flatResultsList = useMemo(() => {
    ...
    if (activeCategory === 'all' || activeCategory === 'songs') {
      filteredResults.tracks.forEach((t) =>
        list.push({ id: t.id, title: t.title, subtitle: t.artist, type: 'track', raw: t })
      );
    }
    if (activeCategory === 'all' || activeCategory === 'albums') {
      filteredResults.albums.forEach((al) =>
        list.push({ id: al.id, title: al.title, subtitle: al.artistName, type: 'album', raw: al })
      );
    }
    if (activeCategory === 'all' || activeCategory === 'artists') {
      filteredResults.artists.forEach((ar) =>
        list.push({ id: ar.id, title: ar.name, subtitle: ar.genres.join(', '), type: 'artist', raw: ar })
      );
    }
    if (activeCategory === 'all' || activeCategory === 'playlists') {
      filteredResults.playlists.forEach((pl) =>
        list.push({ id: pl.id, title: pl.name, subtitle: 'PLAYLIST', type: 'playlist', raw: pl })
      );
    }
  ...
  const triggerAction = (item: {
    ...
    if (item.type === 'track') {
      ...
      setSelectedItem({ id: item.id, type: item.type as any });
    }
  };
  ...
  return React.createElement(
    'div',
    { className: 'flex flex-col gap-ch-8 outline-none', onKeyDown: handleKeyDown },
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('input', {
          ...
          className:
            'w-full h-11 pl-ch-11 pr-ch-4 rounded-ch-lg bg-chotify-bg-surface border border-chotify-border-primary text-xs text-chotify-text-primary placeholder-chotify-text-muted focus:border-chotify-aura-gold/50 outline-none transition-colors focus-ring',
          ...
          categoriesList.map((cat) =>
            React.createElement(
              'button',
              {
                ...
                className: `px-ch-3 py-1.5 rounded-ch-sm text-[10px] font-mono tracking-wider cursor-pointer outline-none focus-ring ${
    ...
    query.trim()
      ? React.createElement(
          'div',
          { className: 'flex-1 flex flex-col gap-ch-6' },
          flatResultsList.length === 0
            ? React.createElement(
                'div',
                { className: 'text-center py-12 text-sm font-mono text-chotify-text-muted' },
                'NO_MATCHING_RESOURCES_FOUND'
              )
            : React.createElement(
                'div',
                { className: 'flex flex-col gap-ch-4' },
                React.createElement(
                  'div',
                  { className: 'text-[10px] font-mono text-chotify-text-muted tracking-widest' },
                  `SEARCH RESULTS (${flatResultsList.length})`
                ),
                React.createElement(
                  'div',
                  ...
                  flatResultsList.map((item, idx) => {
                    ...
                    return React.createElement(
                      'div',
                      {
                        ...
                        className: `flex items-center justify-between p-ch-3 rounded-ch-md cursor-pointer transition-all ${
                          isKeyboardSelected
                      ...
                      React.createElement(
                        'div',
                        { className: 'flex items-center gap-ch-4' },
                        React.createElement(
                          'span',
                          { className: 'text-[10px] font-mono text-chotify-text-muted uppercase w-14' },
                          `[ ${item.type} ]`
                        ),
                        React.createElement(
                          'div',
                          ...
                          React.createElement(
                            'div',
                            { className: 'text-[9px] font-mono text-chotify-text-muted uppercase' },
                            highlightText(item.subtitle, query)
                          )
                        )
                      ),
                      React.createElement(ArrowRight, { className: 'w-ch-3.5 h-ch-3.5 text-chotify-text-muted opacity-45' })
                    );
                  })
                )
              )
      ...
      : // Discovery View (empty search query)
        React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  'button',
                  {
                    ...
                    className:
                      'flex items-center gap-ch-1 text-[10px] font-mono text-chotify-error hover:underline cursor-pointer outline-none',
                  },
                  React.createElement(Trash2, { className: 'w-ch-3 h-ch-3' }),
                  'CLEAR_ALL'
                )
              ),
              React.createElement(
                'div',
                { className: 'flex flex-wrap gap-ch-2 mt-ch-1' },
                recentSearches.map((term) =>
                  React.createElement(
                    'button',
                    {
                      ...
                      className:
                        'flex items-center gap-ch-2 px-ch-3 py-1.5 rounded-ch-sm bg-chotify-bg-surface border border-chotify-border-primary hover:border-chotify-aura-gold/20 text-xs text-chotify-text-secondary cursor-pointer outline-none focus-ring',
                    },
                    React.createElement(Clock, { className: 'w-ch-3.5 h-ch-3.5 text-chotify-text-muted' }),
                    term
                  )
                )
              )
          ...
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'grid grid-cols-1 md:grid-cols-3 gap-ch-6' },
              [
                ...
                React.createElement(
                  'div',
                  {
                    ...
                    className:
                      'p-ch-6 rounded-ch-lg bg-chotify-bg-surface border border-chotify-border-primary hover:border-chotify-aura-gold/40 transition-colors flex flex-col gap-ch-4 cursor-pointer',
                  },
                  React.createElement(
                    'div',
                    {
                    ...
                    React.createElement(cat.icon, { className: 'w-ch-4 h-ch-4' })
                  ),
                  React.createElement(
                    'div',
                    null,
                    React.createElement(
                      'h2',
                      { className: 'text-xs font-mono font-bold tracking-widest text-chotify-text-primary mb-1' },
                      `[ ${cat.name} ]`
                    ),
                    React.createElement(
                      'p',
                      { className: 'text-xs text-chotify-text-secondary' },
                      cat.desc
                    )
                  )
                )
          ...
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'grid grid-cols-2 sm:grid-cols-4 gap-ch-6' },
              albums.map((album) =>
